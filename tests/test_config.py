...
class PlateRecognitionConfigTests(unittest.TestCase):
    def test_plate_detector_settings_are_validated_and_model_path_is_local(self) -> None:
        ...
        self.assertEqual(config.motion_post_roll_ms, 700)
        self.assertEqual(config.motion_quiet_ms, 400)
        self.assertEqual(config.motion_changed_pixel_ratio, 0.03)
        self.assertEqual(config.motion_event_max_duration_ms, 4000)
        self.assertEqual(config.max_replay_frames_per_event, 8)
