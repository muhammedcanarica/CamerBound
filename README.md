...
→ UI event-loop'undan bağımsız kamera başına 5 saniyelik bounded RAM rolling ring
...
Detector ve PaddleOCR ayrı worker'larda çalışır; OCR inference sürerken detector round-robin olarak her kameranın en yeni frame'ini işlemeye devam eder. Detector'ın aynı frame'de bulduğu en fazla iki plaka, kamera kapsamındaki ayrı `PlateTrack` nesnelerine IoU öncelikli ve normalize merkez mesafesi fallback'li one-to-one association ile bağlanır. Her detector crop'u kendi `track_id` değerini taşıyan ayrı bir OCR job'ıdır; confirmation, stabilizasyon, OCR candidate geçmişi ve en iyi sonuç track bazında izole edilir. Aynı track'in bekleyen detector-crop işi varken yalnız daha kaliteli yeni kare onun yerini alabilir, bu nedenle bir araç kuyruğu doldurup diğer aracın OCR almasını engellemez. Track ID değerleri tekrar kullanılmaz; timeout sırasında pending OCR varsa yalnız o track'in işi tamamlanana kadar kontrollü kabul edilir, daha geç sonuç stale olarak düşürülür. Track sona erdiğinde yeni historical OCR evidence üretilmez. Track'in daha önce kuyruğa alınmış pending OCR işleri varsa, track finalization bu işler tamamlanana kadar bekleyebilir. Bu pending OCR sonuçları farklı gerçek frame'lerden yeterli temporal evidence oluşturursa normal confirmation/stabilization hattı üzerinden kayıt yapılabilir. Track sonunda yalnız tek temporal observation kalırsa kayıt yapılmaz; karar AMBIGUOUS_DISCARDED olarak sonuçlanır. PreDetectionFrameBuffer ve historical replay genel detector recovery / motion iş akışlarında kullanılmaya devam edebilir fakat track-end persistence için yeni confirmation evidence üretmez.

Üretilen OCR job'ları kamera başına yapılandırılmış bounded RAM kapasitesinde tutulur. ROI fallback job'ları OCR kuyruğunda 5000 ms, gerçek detector crop'ları ise uzun bir non-preemptive fallback sırasında kaybolmamaları için bounded olarak 12000 ms bekleyebilir. Job türleri açıkça `DETECTOR_CROP`, `DETECTOR_ERROR_FALLBACK`, `ZERO_DETECTION_FALLBACK` ve en düşük öncelikli `STATIC_ZERO_DETECTION_RESCUE` olarak sınıflandırılır. Tüketici yüksek öncelikli detector crop'larından başlayarak işler; her öncelik seviyesinde ENTRY/EXIT round-robin adaleti korunur. Buffer dolduğunda düşük öncelikli job önce evict edilir; yalnız aynı öncelikte detector confidence, crop alanı ve Laplacian sharpness kullanan ucuz kalite skoru karşılaştırılır. Kamera başına her fallback türü bounded tutulur, böylece tekrar eden fallback'ler gerçek detector crop'larını dolduramaz. OCR job, crop ile birlikte aynı immutable full camera frame referansını ve frame zamanlarını taşıdığı için confirmation gözlem zamanı ile confirmed-record JPEG'i doğru frame'e bağlı kalır. Video veya sürekli frame kaydı yapılmaz.
...
Detector bbox'ı yalnız `DETECTOR_CANDIDATE` kanıtıdır; ADMIN preview bunu `DET <confidence>%` olarak gösterir. Crop PaddleOCR sonucunda geçerli Türk plakası üretirse aday `OCR_VERIFIED_PLATE` seviyesine geçer. Birincil detector crop'larının hiçbiri minimum confidence üzerinde geçerli aday üretmezse aynı frame'in ROI'sinde en fazla üç text-detection + recognition search tile çalışır. Böylece far/sis lambası gibi yanlış bir raw bbox, `detections != 0` olduğu için bounded OCR recovery'yi körleştirmez; geçerli detector-crop fast path ek inference almaz.

Detector candidate seçimi confidence ile birlikte bbox aspect, minimum yararlı piksel boyutu ve ROI alan oranından türetilen yumuşak geometry quality kullanır. Bu sinyal hard reject değildir; verified saha bbox dağılımının dışındaki adaylar korunur fakat daha makul tek-satır plaka bbox'larının arkasına düşebilir. Geometry referansı standart tek-satır otomobil/kamyon plakaları içindir; motosiklet ve özel çok-satır plaka yerleşimleri doğrulanmış destek kapsamına dahil değildir.
...
Detector öncesindeki rolling ring her kamera için son 5000 ms ve en fazla 40 full-resolution frame ile sınırlıdır. ROI, yaklaşık 160 piksel genişliğe küçültülüp grayscale/blur/absdiff ile ucuz bir değişen-piksel oranı hesaplanır. Motion event 1500 ms pre-roll, 1500 ms post-roll ve 1000 ms quiet hysteresis kullanır; event en fazla 6000 ms sürer. Event frame'leri zamansal bin'lere bölünür ve her bin içindeki en keskin ROI seçilerek en fazla 10 historical frame replay edilir. Detector 2 live frame / 1 replay frame oranıyla iki kaynağı dengeler; replay kuyruğu kamera başına 2 event ve 8000 ms scheduling yaşı ile bounded'dır.
...
`plate_detection` seviyesindeki temel tracking/buffer/CPU ayarları `max_active_plate_tracks_per_camera=2`, `plate_track_timeout_ms=3000`, `plate_track_iou_threshold=0.25`, `max_pending_ocr_jobs_per_camera=5`, `ocr_job_max_age_ms=5000`, `detector_crop_ocr_job_max_age_ms=12000`, `ocr_cpu_threads=4`, `pre_detection_buffer_duration_ms=5000`, `pre_detection_buffer_max_frames_per_camera=40`, `max_replay_frames_per_event=10`, `max_pending_replay_events_per_camera=2` ve `replay_event_max_age_ms=8000` değerleridir. Track limiti dolduğunda üçüncü eşleşmeyen detection mevcut sağlıklı track'leri evict etmeden atlanır. 12 FPS analysis ingest'te 40 frame yaklaşık 3,25 saniyelik timestamp span sağlar; daha düşük ingest hızında duration limiti 5 saniyede devreye girer. Debug detector kutusu son live detection güncellemesinden 500 ms sonra preview üzerinde çizilmez; `debug_overlay=true` ve ADMIN oturumunda kutular `T1` veya OCR kanıtı oluştuğunda `T1 | 34ABC123 | 0.88` biçiminde gösterilir. `debug_overlay=false` production davranışı değişmez.
...
Yalnız ilk canlı detector-hit frame'inin eşleşen `-full.jpg` ve exact configured ROI `-roi.jpg` çifti `debug/recognition-frames/` altına yazılır; boş yol kareleri ve replay detector sonuçları tek-shot isteğini tüketmez. Klasör Git tarafından yok sayılır ve özellik varsayılan olarak kapalıdır. DEBUG OCR diagnostics, worker aşaması, job türü, fallback nedeni, raw OCR segmenti, normalize/valid aday sonucu ve nihai rejection state'ini birlikte raporlar.

Uygulama zaten çalışıyorsa ADMIN `OCR Tanılama` bölümündeki yön-hedefli `İlk ENTRY/EXIT Detector Karesini Kaydet` düğmeleri aynı tek-shot kaydı runtime'da armar; istek araç gelene kadar bekler. Boş frame, diğer kamera ve replay sonucu tetikleyiciyi tüketmez; kamera/OCR worker yeniden başlatılmaz. Dosya adı ve INFO capture satırındaki `frame_id`, aynı frame'in detector/queue/OCR satırlarıyla kesin eşleşir. Capture edilen tek frame için bu iki ayrıntılı satır INFO seviyesinde de üretilir ve diagnostic throttle bypass edilir; normal frameler yalnız throttled DEBUG olarak kalır.
